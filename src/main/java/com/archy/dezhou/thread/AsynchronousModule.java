...
//	}
	...
	public static boolean isvalidEmail(String email, String[] userinfo)
	{
		...
		List<UserInfo> selectEmailList = session.selectOne("ndb.userValidEmail", email);
...
//					scopeType);
	...
	public void GetUserQueryConsume(User user)
	{
		...
		if (parms.get("status").equals("1102"))
			response.put("st", "ok");
		else
			response.put("st", "fail");
		response.put("code", parms.get("status"));

		response.put("detail", parms.get("detail"));
...
//			PlayerManager.updateMessageQuene2Xml(user, "bbs");
	...
	public void GetUserQueryCharge(User user)
	{
		...
		if (parms.get("status").equals("1104"))
		{
			response.put("st", "ok");
		}
		else
...
//			PlayerManager.updateMessageQuene2Xml(user, "bbs");
	...
	public void GetUserQueryBalance(User user)
	{
		...
		if (parms.get("status").equals("1105"))
		{
			response.put("st", "ok");
			response.put("gold", parms.get("point"));
			response.put("code", parms.get("status"));
		}
		else
		{
...
//					PlayerManager.updateMessageQuene2Xml(user, "bbs");
			...
			else
			{
				...
				parmsMap.put("hRet", parms.get("hRet") + "");
