package com.archy.texasholder.thread.roomUnit;

import java.util.List;
import java.util.concurrent.TimeUnit;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Component;

import java.util.ArrayList;

import com.archy.texasholder.entity.room.PukerGame;
import com.archy.texasholder.service.RoomService;
import com.archy.texasholder.thread.roomUnit.base.IRoomDealUnit;

import jakarta.annotation.Resource;

@Component
public class RoomDealUnit implements IRoomDealUnit
{

	private Logger log = LoggerFactory.getLogger(getClass());

	@Resource
	private RoomService roomService;
	
	// @Scheduled(fixedDelay = 10, initialDelay = 1000, timeUnit = java.util.concurrent.TimeUnit.MILLISECONDS)
	@Scheduled(fixedDelay = 1000, initialDelay = 5000, timeUnit = TimeUnit.MILLISECONDS)
	public void heartbeat()
	{

		// log.info("RoomDealUnit heartbeat");
		long start = System.currentTimeMillis();

		List<PukerGame> roomListCpy = new ArrayList<PukerGame>();
		roomListCpy.addAll(roomService.getRoomList());
		roomListCpy.forEach(room -> {
			room.beatHeart(start);
		});


		long end = System.currentTimeMillis();
		if((end - start) < 30)
		{
			try
			{
				Thread.sleep(10);
			}
			catch (Exception e)
			{
				log.error("Error",e);
			}
		}
	}


}
