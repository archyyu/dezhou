...
import com.yl.room.base.IRoomState;
...
//		}
		...
		if(this.isGame())
		{
			player.setGameState(PlayerGameState.PLAYER_STATE_WAIT);
			player.setPlayerState(PlayerCareerState.PLAYER_STATE_WAIT);
		}
		else
		{
			player.setGameState(PlayerGameState.PLAYER_STATE_PLAYER);
			player.setPlayerState(PlayerCareerState.PLAYER_STATE_PLAYER);			
		}
		...
		this.notifyRoomPlayerButOne(response,MessageType.MESSAGE_NINE,user.getUid());
		return response;
	}
	...
	public ActionscriptObject userStandUp(User u,boolean notifyMySelf)
	{
		...
		if(notifyMySelf)
		{
			this.notifyRoomPlayer(response,MessageType.MESSAGE_NINE);
		}
		else
		{
			this.notifyRoomPlayerButOne(response, MessageType.MESSAGE_NINE, u.getUid());
		}
		return response;
	}
	...
	public String getXmlVariable(String varName)
	{
		...
		RoomVariable rv = (RoomVariable) roomVariables.get(varName);
		if (rv != null)
		{
	...
	{
		return (RoomVariable) roomVariables.get(varName);
	}
