...
public class PayUtils
{
	...
	{
		return getExchangeMap().containsKey(money) ? true : false;
	}
	...
	{
		return PramValueList().contains(pmethod) ? true : false;
	}
	...
	{
		return getGtypeByAmt(amt) == 0 ? true : false;
	}
	...
	{
		return getGtypeByMyr(amt) == 0 ? true : false;
	}
			...
			String pmoney)
	{
		...
		else
		{
			...
			{
				flag = checkAmtByMyr(pmoney) ? true : false;
			}
			else if (pmethod.equals(PayFinalValue.PV_PAY_PAYPAL_Paypal))
			{
				flag = checkAmtByDollar(pmoney) ? true : false;
			}
			...
			{
				flag = checkPayMoney(pmoney) ? false : true;
			}
		}
		return flag;
	}
